#!/usr/bin/env bash
# Build Tycheism II for arXiv: Markdown -> LaTeX -> PDF
#
# Requires: pandoc, pdflatex (MiKTeX or TeX Live)
# Produces: paper_b.tex, paper_b.pdf
# Zip for arXiv upload after build succeeds:
#   cd arxiv/paper_b && zip paper_b.zip paper_b.tex
...
TITLE="Tycheism II: Trajectory Sovereignty as Epistemic Necessity"
AUTHOR="Mikhail Arbuzov"

# XeLaTeX is used because the trilogy uses Greek Unicode (Τύχη).

echo "[1/3] pandoc: paper_b.md -> paper_b.tex"
pandoc paper_b.md \
...
xelatex -interaction=nonstopmode -halt-on-error paper_b.tex > /dev/null

echo "[3/3] xelatex pass 2 (for cross-refs)"
xelatex -interaction=nonstopmode -halt-on-error paper_b.tex > /dev/null

echo "Build complete: paper_b.pdf"
ls -la paper_b.pdf
