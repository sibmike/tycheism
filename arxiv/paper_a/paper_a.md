...
This is the first paper in a three-part series. The framework is called *Tycheism*, after Tyche (Τύχη), the Greek goddess of fortune. Tyche is not a metaphor for stochastic noise. She is the irreducible component of the coupling loop that makes every trajectory distinct. The agent chooses the action; Tyche returns the outcome. Without Tyche's returns, two agents with identical states on identical landscapes would walk identical trajectories forever — the coupling loop would run, but nothing would differentiate one run from another. Tyche is what makes the trajectory the agent's own. Formally, Tyche names the realized encounter $e_{t+1}$ — the single Bernoulli draw at each visited cell — whenever we discuss it as a figure rather than as a variable. The rest of the paper uses "Tyche" and "$e_{t+1}$" interchangeably.

Tycheism inherits from but is distinct from Charles Sanders Peirce's *tychism* (1892). Peirce's tychism is a cosmological claim: chance is ontologically real, laws of nature are evolved habits, and indeterminacy is a genuine feature of the universe. Tycheism takes this ontological-chance premise as foundational but develops a further claim Peirce did not approach: when genuine chance — Tyche's returns — is processed through coupled learning, it produces trajectories that are constitutively non-fungible. Over any meaningful horizon, no two agents receive the same sequence of returns, even on the same landscape with the same process. The trajectory each agent walks is singular. The spelling marks the difference: Peirce named the cosmological property; Tycheism is built on the figure who personifies it.

The encounter at each grid cell is a genuine coin flip: $+1$ with probability $p(x)$, $-1$ with probability $1 - p(x)$. This is not Gaussian noise on a deterministic signal. The outcome is undetermined until the coin is flipped. The agent never observes $p(x)$ directly — it learns only by visiting cells and receiving what Tyche returns. The model instantiates the ontological-chance premise directly: the landscape is hidden, the coins are real, the agent's learning is shaped by genuine indeterminacy, amplified through the coupling loop. Paper II argues moral development is precisely this kind of process. Paper III examines what the ambient architecture of aligned AI does to it.
...
![**Figure 1.** The coupling loop. $Q$-values drive softmax arm selection $\pi$; the chosen arm $a$ determines the visited cell $x$; Tyche's return $e \in \{+1, -1\}$ updates $Q$ at that cell. The loop closes: encounters shape the state that selects future encounters. The single source of external indeterminacy is the Bernoulli draw at step 3.](figures/figure1_coupling_loop.png)

**The role of Tyche in the loop.** The encounter step (step 3) is the only step in the loop that is not determined by the agent's state or the model's structure. The state selects the cell; the bias $p(x)$ is a property of the landscape; the update rule is fixed. But the actual outcome $e_{t+1} \in \{+1, -1\}$ is Tyche's return. Over a horizon of $H$ timesteps, Tyche's returns constitute a sequence of $H$ Bernoulli outcomes. The space of possible sequences has cardinality $2^H$; for $H = 400$, this is approximately $10^{120}$. The probability that two independent runs share an identical sequence of outcomes at a shared sequence of cells is bounded above by $2^{-H}$ and is astronomically small for any non-degenerate $p(x)$ — over meaningful horizons, Tyche's returns are not reproducible across runs, and the coupling loop carries her non-reproducibility forward into the agent's learned state. Trajectory divergence between any two agents is not a contingent outcome of the experiment. It is the guaranteed consequence of Tyche's returns being singular, processed through a loop that amplifies singular input into singular state.

This distinguishes Tycheism's claim from Peirce's tychism. Peirce argued chance is real. Tycheism adds: real chance, processed through coupling, makes every trajectory constitutively non-fungible. No agent can traverse another's path, not even in principle, because the path is partially constituted by returns only that agent received.
...
![**Figure 2.** Landscape gallery. Colour encodes the hidden coin bias $p(x) \in [0, 1]$; lighter cells have higher bias. Left: smooth single-peak landscape used in Experiments 1–3 and 6. Centre: two-island landscape used in Experiment 4b (two high-bias peaks at $(10, 10)$ and $(30, 30)$, low-bias gap between them). Right: a rugged multi-modal landscape shown for comparison; not used in the experiments reported here. The agent never observes $p(x)$ directly — it learns only through visited-cell coin flips.](figures/figure2_landscape_gallery.png)
...
![**Figure 3.** Panel (a): $D_Q(t)$ for the coupled agent (rising curve with confidence band) and the TvL baseline (flat at zero). Panel (b): Terminal $D_Q$ by number of forced encounters.](figures/figure3_intervention_cascade.png)
...
![**Figure 3b.** Spatial divergence timeline: six snapshots showing both agents' positions on the landscape at $t_0$, $t_0 + 25$, $t_0 + 70$, $t_0 + 140$, $t_0 + 220$, $t_0 + 330$, with $D_Q$ annotated on each panel.](figures/figure3b_divergence_timeline.png)
...
![**Figure 4.** 2×2 decomposition matrix showing $D_Q$ by condition (learn-yes / learn-no × use-yes / use-no). Only the two learn-yes conditions produce nonzero $D_Q$; learning is necessary for Q-map divergence, and coupling converts representational divergence into behavioural divergence.](figures/figure4_coupling_decomposition.png)
...
![**Figure 5.** Panel (a): Two-island landscape with smoothed trajectory overlay — the endogenous agent (blue) stays near peak A, the intervened agent (orange) migrates to peak B. Panel (b): $D_Q(t)$ showing rapid rise to plateau after the intervention at $t_0 = 100$. Panel (c): Switching rates by operator with 95% confidence intervals.](figures/figure5_island_attractors.png)
...
This result is confined, in this paper, to a spatial bandit with two islands. Whether real developmental landscapes — moral, epistemic, practical — are single-peaked or multi-modal is, at this point, genuinely an open empirical question, one this paper deliberately does not attempt to answer. But any domain where reasonable agents can end up in genuinely different places while doing everything right is a domain where this result applies. That is the question Paper II takes up.
...
![**Figure 6.** Horizontal bar chart of intervention operators ranked by terminal divergence $D_Q(H)$, colour-coded by coupling status (preserved / weakened / broken). Three clusters emerge: low (Option Expansion), moderate (five operators including partial State Replacement, Option Removal, Selection Bias, Encounter Arrangement, Learning Impairment), and very high (full State Replacement).](figures/figure6_operator_ranking.png)
...
That argument is precisely the work of Paper II.
...
On a landscape with more than one valid destination, this mechanism does not produce convergence to the right answer. It produces convergence to an answer — selected by stochastic history, locked in by the agent's own learning. Two agents, same landscape, same process, same integrity of mechanism, separated by one coin flip. Each coin flip is Tyche's return — the one piece of the loop the agent could not author. Neither is wrong. That is what coupling does.
