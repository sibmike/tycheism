#!/usr/bin/env bash
# Build Tycheism I for arXiv: Markdown -> LaTeX -> PDF
#
# Requires: pandoc, pdflatex (MiKTeX or TeX Live)
# Produces: paper_a.tex, paper_a.pdf
# Zip for arXiv upload after build succeeds:
#   cd arxiv/paper_a && zip -r paper_a.zip paper_a.tex figures/
...
TITLE="Tycheism I: Trajectory Divergence in Coupled Stochastic Learning Systems"
AUTHOR="Mikhail Arbuzov"

# XeLaTeX is used because the paper contains Greek Unicode (Τύχη) that pdflatex
# cannot handle without explicit \DeclareUnicodeCharacter declarations.

echo "[1/3] pandoc: paper_a.md -> paper_a.tex"
pandoc paper_a.md \
...
xelatex -interaction=nonstopmode -halt-on-error paper_a.tex > /dev/null

echo "[3/3] xelatex pass 2 (for cross-refs)"
xelatex -interaction=nonstopmode -halt-on-error paper_a.tex > /dev/null

echo "Build complete: paper_a.pdf"
ls -la paper_a.pdf
