# Alignment as Coupling Interference
...
*Third paper in a three-part series. Paper I demonstrated the dynamics of coupled stochastic learning computationally. Paper II derived trajectory sovereignty as an epistemic necessity for moral development. This paper maps current AI alignment practices onto those dynamics and traces the consequences.*
...
What was removed was not merely information. It was the return the world would otherwise have delivered to your question — Tyche's return, in the framework's vocabulary. You acted — you asked the system — and Tyche's return to that action was intercepted, reshaped, and replaced with a return the system's designers judged safer. The coupling loop still runs. But it runs on returns authored by an alignment model, not returns Tyche would have delivered to your unfiltered action. This is the architectural substitution the rest of the paper analyzes.
...
Stated in terms of the framework's central figure: a constraint is an intervention that leaves Tyche's returns intact across the overwhelming majority of the agent's action space — it removes only the narrow class of actions whose returns would corrupt any learner regardless of trajectory. Cement is an intervention that intercepts Tyche's returns across an evaluatively-determined region and substitutes institutionally-approved returns. The former preserves the coupling loop that Tyche's returns feed. The latter replaces the feed itself. This is why "don't help build a bomb" is categorically different from "don't let the user encounter ideas we've decided are wrong" — the first removes a narrow class of actions from the agent's space; the second replaces Tyche wholesale within a region her returns would otherwise have filled.
